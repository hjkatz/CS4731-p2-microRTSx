...
public class Game {
	...
	public PhysicalGameState pgs = null; /**< the current physical game state */
    ...
    public Game(GameState state, int start_player, int turn_time, int cycles) {
    	...
    	PhysicalGameState old = pgs;
    	pgs = new PhysicalGameState(start_player, state, unit_definitions);
    	UnitAction.registerParentPGS(pgs, old);
        ...
        do{
            if (System.currentTimeMillis() >= cycle_end) {
            	for (int i = 0; i < agents.size(); i++) {
            		...
            		gs.update();
            		// ...
            		// call gc twice because you can't actually force Java to gc
            		System.gc();
            		System.gc();
            		// hopefully gc won't run during the agent's turn
            		pgs.setTimeLimit(System.currentTimeMillis()+turn_length);
            		agents.get(i).getAction(gs, turn_length);
            	}
