...
public class UnitAction {
	private static PhysicalGameState parent_pgs; /**< used to ensure the "public private" methods aren't accessed */
	...
	/**
	 * Internal use only. Returns whether or not this action is ready to execute
	 * @return
	 */
	public boolean ready(PhysicalGameState pgs) {
		if (!pgs.equals(parent_pgs)) {
			return false;
		}
	 ...
	 * @return
	 */
	public boolean cooldown(PhysicalGameState pgs) {
		if (!pgs.equals(parent_pgs)) {
			return false;
		}
		return(--cooldown<=0);
	...
	/**
	 * Internal use only. Validates this action as legal or not
	...
	/**
	 * Internal use only. Executes this action
	 * @param unit the unit this action is for
...
//			}
	 ...
	 *  Internal use only
	 */
	public static void registerParentPGS(PhysicalGameState pgs, PhysicalGameState old_parent_pgs) {
		if ((parent_pgs == null && old_parent_pgs == null) || old_parent_pgs.equals(parent_pgs)) {
			parent_pgs = pgs;
		}
	}
}
