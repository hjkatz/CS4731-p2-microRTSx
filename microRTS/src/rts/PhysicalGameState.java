				...
				if (action.getTimestamp() <= timeLimits.get(armyStats.get(i).player) && action.validate(armyStats.get(i), this)) {
					...
					if (action.ready(this)) {
						if (action.getType() == UnitAction.BUILD) {
							int target = action.getTargetX()+action.getTargetY()*width;
							boolean valid_location = (terrain[target] == TERRAIN_NONE);
							if (!valid_location && armyStats.get(i).definition.is_building) { // building a unit
								// have to check if flying
								valid_location = unitDefinitions.unit_defs.get(current_player+1).get(action.getBuild()).is_flying;
							}
							if (unit_map[target] == null && valid_location) {
								unit_map[target] = reserved;
					...
					if (armyStats.get(i).action != null && action.cooldown(this)) {
						if (action.getType() == UnitAction.BUILD){
...
//    	}
    ...
    /**
     * Determines if two PhysicalGameStates are the same
     *@param pgs the other PhysicalGameState
     *@return true or false
     */
    public boolean equals(PhysicalGameState pgs) {
    	return(pgs == this && pgs.id == id);
    }
}
