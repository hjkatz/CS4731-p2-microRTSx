...
{
    public ArrayList<GeneralAITown> towns;
    ...
    public void manage_units( GeneralAI ai )
    {
        if ( ( ai.getLesion() & GeneralAI.LESION_NO_DEFENSE ) == 0 )
        {
            ...
            {
                if ( towns.get( i ).owner == ai.player_id )
                { // my town
            ...
            if ( units.size() + units_scouted.size() < defenders )
            {
                for ( int i = 0; i < ai.units.size(); i++ )
                {
                    ...
                    if ( !unit.stats.isBuilding() && !unit.stats.isWorker() )
                    { // workers are already in town
            ...
            for ( int i = 0; i < units.size(); i++ )
            {
                ...
                if ( unit.object == null )
                {
                    ...
                    if ( unit.object == null )
                    { // somehow there are no more towns... (?)
