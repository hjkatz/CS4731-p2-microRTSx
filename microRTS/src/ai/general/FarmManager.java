...
{
    public ArrayList<GeneralAIFarm> farms;
    /** < resource fields we know about */
    public ArrayList<GeneralAIFarm> owned_farms;
    /** < a quick listing of all the farms that we "own" */
    
    public int                      workers_wanted;
                ...
                for ( int k = 0; k < farm.farmers.length; k++ )
                {
                    if ( farm.farmers[k] == GeneralAIFarm.FARM_OPEN )
                    {
                        ...
                        for ( int j = 0; j < ai.units.size(); j++ )
                        {
                            ...
                            if ( unit.stats.isWorker() && unit.wanted_strategy != GeneralAI.STRATEGY_FARM && unit.strategy != GeneralAI.STRATEGY_FARM )
                            { // make sure not a unit already scouted
                        ...
                        {
                            if ( closest_unit.strategy == GeneralAI.STRATEGY_NONE )
                            { // we can recruit him now!
