...
public class GeneralAI extends AI
{
    ...
    /** < no strategy */
    public static final int         STRATEGY_BUILD        = 1;
    /** < build */
    public static final int         STRATEGY_FARM         = 2;
    /** < gather resources */
    public static final int         STRATEGY_ATTACK       = 3;
    /** < attack enemy */
    public static final int         STRATEGY_EXPLORE      = 4;
    /** < explore the map strat */
    public static final int         STRATEGY_DEFENSE      = 5;
    /** < defend a position */
    
    public static final int         DISTANCE_IGNORE       = -1;
    /** < ignore this distance */
    
    private boolean                 init;
    /** < whether or not we have init */
    
    private long                    turn_start;
    /** < when the turn started */
    private long                    turn_limit;
    /** < maximum length for turn */
    
    public TrafficMap               traffic_map;
    /** < a map of our unit traffic */
    
    public ExplorationManager       exploration_manager;
    /** < exploration manager */
    public ProductionManager        production_manager;
    /** < production manager */
    public AttackManager            attack_manager;
    /** < attack manager */
    public FarmManager              farm_manager;
    /** < farm manager */
    public TownManager              town_manager;
    /** < town manager */
    ...
    public ArrayList<GeneralAIUnit> units;
    /** < my units */
    ...
    {
        if ( ( exploration_manager.map[location] & ( GameState.MAP_NEUTRAL | GameState.MAP_NONPLAYER ) ) == 0 && ( ( exploration_manager.map[location] & GameState.MAP_WALL ) == 0 || unit.isFlying() )
                && traffic_map.valid( location, turn_start, turn_end ) )
        {
            return true;
        }
        return false;
    ...
    public ArrayList<Integer[]> get_path( Unit unit, int start, int turn_start, ArrayList<Integer> destinations )
    {
        ...
        while ( open.size() > 0 )
        {
            ...
            next = current + 1;
            if ( cx < state.getMapWidth() - 1 && ( destinations.contains( next ) || ( !closed.contains( next ) && can_enter( unit, next, time, end ) ) ) )
            { // right exists
                if ( !open.contains( next ) || next_g < g_score.get( next ) )
                {
                    came_from.put( next, current );
            ...
            next = current + state.getMapWidth();
            if ( cy < state.getMapHeight() - 1 && ( destinations.contains( next ) || ( !closed.contains( next ) && can_enter( unit, next, time, end ) ) ) )
            { // down exists
                if ( !open.contains( next ) || next_g < g_score.get( next ) )
                {
                    came_from.put( next, current );
