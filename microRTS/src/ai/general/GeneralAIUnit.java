...
/**
 * \brief An AI unit used by the GeneralAI. Allows for more sophisticated
 * behaviors, like order queuing.
 * 
 * @author Jeff Bernard
 * 
 */
public class GeneralAIUnit
{
    ...
    /** < whether or not this unit exists */
    public ArrayList<UnitAction> actions;
    /** < pending actions */
    ...
    /** < the last traffic reservation */
    
    public int                   strategy;
    /** < the strategy this unit is adhering to */
    
    private boolean              hasActed;
    /** < whether or not this unit has executed at least 1 action */
    
    public GeneralAIObject       object;
    /** < the object this unit is interacting with */
    public boolean               remove_object;
    /** < whether or not this unit should try and remove it's object */
    
    public int                   resources_held;
    /** < how many resources this guy previously held */
    
    public int                   wanted_strategy;
    /** < the strategy of the manager trying to recruit this unit */
    
    public ArrayList<Long>       dont_attack;
    
    /** < list of enemies that we cannot reach */
     ...
     * 
     * @param ai
     *            the general ai that is controlling this unit
     */
     ...
     * @param ai
     *            the ai
     * @return true if the next action was performed, otherwise false (no
     *         actions?)
     */
    ...
    public float evaluate()
    {
        ...
        return ( ( ( stats.getAttackMin() * stats.getAttackRange() + stats.getVision() ) * ( ( float ) stats.getHP() / ( float ) stats.getMaxHP() ) + ( stats.isFlying() ? 1 : 0 ) ) / ( float ) ( stats
                .getAttackSpeed() + stats.getMoveSpeed() ) );
    }
    ...
    public ArrayList<Integer> adjacent_build_locations( GeneralAIProduction product, GeneralAI ai )
    {
        ...
        if ( stats.getX() < ai.state.getMapWidth() - 1
                && ( ( ai.exploration_manager.map[stats.getX() + 1 + stats.getY() * ai.state.getMapWidth()] & GameState.MAP_WALL ) != 1 || product.def.is_flying ) )
        {
            locations.add( stats.getX() + 1 + stats.getY() * ai.state.getMapWidth() );
        }
        ...
        if ( stats.getY() < ai.state.getMapHeight() - 1
                && ( ( ai.exploration_manager.map[stats.getX() + ( stats.getY() + 1 ) * ai.state.getMapWidth()] & GameState.MAP_WALL ) != 1 || product.def.is_flying ) )
        {
            locations.add( stats.getX() + ( stats.getY() + 1 ) * ai.state.getMapWidth() );
        }
        return locations;
