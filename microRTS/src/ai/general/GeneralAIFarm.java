...
{
    public static final int FARM_CLOSED    = -1;
    /** < the farm is closed */
    public static final int FARM_OPEN      = -2;
    /** < the farm is open */
    public static final int FARM_WALL_OPEN = -3;
    /** < the farm is open to fliers */
    public static final int FARM_UNKNOWN   = -4;
    /** < nobody knows what's going on at the farm !? */
    
    public static final int FARMER_UP      = 0;
    /** < the farmer in the up spot */
    public static final int FARMER_DOWN    = 1;
    /** < the farmer in the down spot */
    public static final int FARMER_LEFT    = 2;
    /** < the farmer in the left spot */
    public static final int FARMER_RIGHT   = 3;
    /** < the farmer in the right spot */
     ...
     * 
     * @param unit
     *            the unit to check if the farm has an opening for
        ...
        { // is possible to reach goal
            ...
            if ( !there )
            {
                for ( int i = rpath.size() - 1; i >= 0; i-- )
                {
                    ...
                    unit.addAction( new UnitAction( unit.stats, UnitAction.MOVE, rpath.get( i )[0] % ai.state.getMapWidth(), rpath.get( i )[0] / ai.state.getMapWidth(), -1 ), ai.traffic_map,
                            rpath.get( i )[0], rpath.get( i )[1], rpath.get( i )[1] + unit.stats.getMoveSpeed() );
                }
            }
    ...
    private void path_to_stockpile( GeneralAIUnit unit, GeneralAI ai, int now_at, int now_start )
    {
        ...
        if ( rpath != null )
        {
            ...
            unit.addAction( new UnitAction( unit.stats, UnitAction.RETURN, rpath.get( 0 )[0] % ai.state.getMapWidth(), rpath.get( 0 )[0] / ai.state.getMapWidth(), -1 ), ai.traffic_map, now_at,
                    now_start, now_start + UnitAction.DEFAULT_COOLDOWN );
        }
        else
        {
        }
