...
public class GeneralAIProduction extends GeneralAIObject
{
    ...
    public int            priority;
    /**
     * < how badly we want this unit. -1 is not all, 0 is most of all, > 0 sort
     * of want
     */
    
    public float          cost_ratio;
    /** < cost of building this unit, with respect to it's fight utility */
     ...
     * @param definition
     * @param _id
     *            the index in unit/building list
     * @param _x
     * @param _y
     */
    ...
    public void order_unit( GeneralAIUnit unit, GeneralAI ai )
    {
        if ( def.is_building )
        {
            ...
            if ( y < ai.state.getMapHeight() - 1
                    && ( ai.exploration_manager.map[x + ( y + 1 ) * ai.state.getMapWidth()] & ( GameState.MAP_WALL | GameState.MAP_NEUTRAL | GameState.MAP_NONPLAYER ) ) == 0 )
            {
                destination.add( x + y * ai.state.getMapWidth() + ai.state.getMapWidth() );
            }
