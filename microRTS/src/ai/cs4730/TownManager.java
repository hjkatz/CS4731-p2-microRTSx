...
public class TownManager extends Manager
{
        ...
        for ( WorkerUnitController worker : ai.workers )
        {
            ...
            if ( worker.actions.size() <= 0 ) // no actions
            {
                if ( worker.unit.getHarvestAmount() <= 0 )
                {
                    ...
                    if ( farm != null )
                    {
                        pathToFarm( ai, worker, farm );
                    }
                }
                else
                {
                    pathToStockpile( ai, worker, worker.getY() * MapUtil.WIDTH + worker.getX(), ai.currentTurn );
                }
            }
        }
        
        for ( BuildingUnitController stock : ai.stockpiles )
        {// all the bases, tell em to make workers
            stock.act( ai );
            
            if ( ai.workers.size() + ai.scouts.size() < ai.wantedScouts + ai.wantedWorkers )
            {
                if ( stock.actions.size() <= 0 )
                { // no actions?!?!?
                    int time = ai.currentTurn;
                    int position = stock.getY() * MapUtil.WIDTH + stock.getX() + 1;
                    stock.addAction( new UnitAction( stock.unit, UnitAction.BUILD, stock.getX(), stock.getY() + 1, ai.WORKER ), MapUtil.trafficMap, position, time, time
                            + ai.workers.get( 0 ).getBuildSpeed() );
    ...
    private void pathToFarm( AIController ai, WorkerUnitController worker, FarmUnitController farm )
    {
        ...
        { // is possible to reach goal
            ...
            if ( !there )
            {
                ...
                {
                    worker.addAction( new UnitAction( worker.unit, UnitAction.MOVE, rpath.get( i )[0] % MapUtil.WIDTH, rpath.get( i )[0] / MapUtil.WIDTH, -1 ), MapUtil.trafficMap, rpath.get( i )[0],
                            rpath.get( i )[1], rpath.get( i )[1] + worker.unit.getMoveSpeed() );
                }
            }
            position = rpath.get( 0 )[0];
            time = rpath.get( 0 )[1];
        ...
        pathToStockpile( ai, worker, position, time );
    }
    
    private void pathToStockpile( AIController ai, WorkerUnitController worker, int position, int time )
    {
