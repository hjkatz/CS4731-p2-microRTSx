...
public class BuildingManager extends Manager
{
    ...
    public void update()
    {
        ...
        {// all the bases, tell em to make workers
            ...
            { // no actions?!?!?
                if ( ai.farmers.size() + ai.scouts.size() < ai.wantedScouts + ai.wantedWorkers + ai.wantedBuilders )
                {// do we want scouts?
                    if ( canAffordUnit( AIController.WORKER ) )
    ...
    public void buildUnit( BuildingUnitController building, int unitType )
    {
        ...
        building.addAction( new UnitAction( building.unit, UnitAction.BUILD, building.getX(), building.getY() + 1, unitType ), MapUtil.trafficMap, position, time, time
                + ai.unitTypes.get( unitType ).produce_speed );
