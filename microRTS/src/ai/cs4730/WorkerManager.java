...
public class WorkerManager extends Manager
{
            ...
            if ( builder.actions.size() <= 0 ) // no actions?!
            {
                ...
                if ( wanted != null )
                {
                    ...
                    { // is possible to reach goal
                        ...
                        {
                            builder.addAction( new UnitAction( builder.unit, UnitAction.MOVE, path.get( i )[0] % MapUtil.WIDTH, path.get( i )[0] / MapUtil.WIDTH, -1 ), MapUtil.trafficMap,
                                    path.get( i )[0], path.get( i )[1], path.get( i )[1] + builder.unit.getMoveSpeed() );
                        }
                        // first item in path is the planned location
                        int position = path.get( 0 )[0];
                        int time = path.get( 0 )[1];
                        
                        builder.addAction( new UnitAction( builder.unit, UnitAction.BUILD, position % MapUtil.WIDTH, position / MapUtil.WIDTH, wanted.unitType ), MapUtil.trafficMap, position, time,
                                time + builder.getBuildSpeed() );
                ...
                {
                    //Roam to other stockpiles
                    ArrayList<Integer> pos = new ArrayList<Integer>();
                    for ( BuildingUnitController stock : ai.stockpiles )
                    {
                        if ( MapUtil.distance( stock, builder ) >= 6 )
                        {
                            pos.add( MapUtil.position( stock ) );
                        }
                    }
    ...
    public FarmUnitController nextFreeFarm()
    {
        for ( FarmUnitController farm : ai.farms )
        {
            if ( farm.isFree() && MapUtil.getSurroundingPositions( MapUtil.position( farm ) ).size() > 0 )
            {
                return farm;
            }
        }
    ...
    private void returnResources( WorkerUnitController worker, int position, int time )
    {
        ...
        if ( path != null )
        {
            for ( int i = path.size() - 1; i >= 1; i-- )
            {
                worker.addAction( new UnitAction( worker.unit, UnitAction.MOVE, path.get( i )[0] % MapUtil.WIDTH, path.get( i )[0] / MapUtil.WIDTH, -1 ), MapUtil.trafficMap, path.get( i )[0],
                        path.get( i )[1], path.get( i )[1] + worker.unit.getMoveSpeed() );
            }
            // first element is the stockpile
            position = path.get( 0 )[0];
            time = path.get( 0 )[1];
            worker.addAction( new UnitAction( worker.unit, UnitAction.RETURN, position % MapUtil.WIDTH, position / MapUtil.WIDTH, -1 ), MapUtil.trafficMap, position, time, time
                    + UnitAction.DEFAULT_COOLDOWN );
        }
    }
    ...
    private void harvestFarm( WorkerUnitController worker, FarmUnitController farm )
    {
        if ( needNewStockpile( farm ) )
        {
            ...
            if ( positions.size() > 0 )
            {
                if ( ai.buildingManager.canAffordBuilding( AIController.STOCKPILE ) )
                {
