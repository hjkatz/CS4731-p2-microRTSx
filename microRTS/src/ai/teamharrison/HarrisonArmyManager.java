...
public class HarrisonArmyManager extends HarrisonManager
{
    ...
    public void update()
    {
        ...
        switch ( state )
        {
                        ...
                        if ( uc.actions.size() <= 0 || !uc.lastActionSucceeded() )
                        {
                            ...
                            for ( UnitAction action : legalActions )
                            {
                                if ( action.getType() == UnitAction.ATTACK && ( HarrisonMapUtil.map[HarrisonMapUtil.position( action.getTargetX(), action.getTargetY() )] & GameState.MAP_PLAYER ) == 0 )
                                {
                                    ...
                                    for ( int i = ai.currentTurn; i < ai.currentTurn + uc.getAttackSpeed() - 1; i++ )
                                    {// i is time
                                        uc.addAction( new UnitAction( uc.unit, UnitAction.NONE, uc.getX(), uc.getY(), -1 ), HarrisonMapUtil.harrisonTrafficMap, HarrisonMapUtil.position( uc ), i,
                                                i + 1 );
                                    }
                                    canAttack = true;
                                    break;
            ...
            case MoveTowardsUnit :
                        ...
                        if ( uc.actions.size() <= 0 || !uc.lastActionSucceeded() )
                        {
                            ...
                            for ( UnitAction action : legalActions )
                            {
                                if ( action.getType() == UnitAction.ATTACK && ( HarrisonMapUtil.map[HarrisonMapUtil.position( action.getTargetX(), action.getTargetY() )] & GameState.MAP_PLAYER ) == 0 )
                                {
                                    ...
                                    {
                                        uc.addAction( new UnitAction( uc.unit, UnitAction.NONE, uc.getX(), uc.getY(), -1 ), HarrisonMapUtil.harrisonTrafficMap, HarrisonMapUtil.position( uc ), time,
                                                time + 1 );
                                        time++;
                                    }
                                    canAttack = true;
                ...
                else if ( ai.enemyBuildings.size() == 0 )
                {
                    targetingPosition = randomPosition();
                    state = STATE.Explore;
                }
                                ...
                                else
                                {
                                    ...
                                    { // is possible to reach goal
                                        if ( path.size() != 0 )
                                        {
                                            ...
                                            {
                                                uc.addAction( new UnitAction( uc.unit, UnitAction.MOVE, path.get( i )[0] % HarrisonMapUtil.WIDTH, path.get( i )[0] / HarrisonMapUtil.WIDTH, -1 ),
                                                        HarrisonMapUtil.harrisonTrafficMap, path.get( i )[0], path.get( i )[1], path.get( i )[1] + uc.unit.getMoveSpeed() );
                                            }
                                        }
                                    }
                                }
                    ...
                    if ( scout.actions.size() <= 0 )
                    {
                        ...
                        if ( HarrisonMapUtil.position( targetX, targetY ) != HarrisonMapUtil.position( scout ) )
                        {
                            ...
                            { // is possible to reach goal
                                for ( int i = path.size() - 1; i >= 0; i-- )
                                {
                                    scout.addAction( new UnitAction( scout.unit, UnitAction.MOVE, path.get( i )[0] % HarrisonMapUtil.WIDTH, path.get( i )[0] / HarrisonMapUtil.WIDTH, -1 ),
                                            HarrisonMapUtil.harrisonTrafficMap, path.get( i )[0], path.get( i )[1], path.get( i )[1] + scout.unit.getMoveSpeed() );
                                }
                            }
                        }
                        else
            ...
            case Explore :
                for ( HarrisonArmyUnitController uc : ai.armyUnits )
                {
                    if ( uc.actions.size() <= 0 )
                    {
                        ...
                        UnitAction action = getPossibleMove( legalActions );
                        if ( action != null )
                        {
    ...
    private UnitAction getPossibleMove( ArrayList<UnitAction> legalActions )
    {
        for ( UnitAction action : legalActions )
        {
