...
public class HarrisonWorkerManager extends HarrisonManager
{
    ...
    private void moveTowards( HarrisonUnitController uc, int dx, int dy )
    {
        ...
        if ( path != null )
        {
            ...
            {
                uc.addAction( new UnitAction( uc.unit, UnitAction.MOVE, path.get( i )[0] % HarrisonMapUtil.WIDTH, path.get( i )[0] / HarrisonMapUtil.WIDTH, -1 ), HarrisonMapUtil.harrisonTrafficMap,
                        path.get( i )[0], path.get( i )[1], path.get( i )[1] + uc.unit.getMoveSpeed() );
            }
        }
    ...
    protected class BuildingRequestedRule extends Rule
    {
        ...
        @Override
        public boolean isTriggered( HarrisonUnitController worker )
        {
        ...
        public boolean isTriggered( HarrisonUnitController worker )
        {
            ...
            if ( distance >= 5 )
            {
                for ( HarrisonWantedUnit wanted : HarrisonUnitQueue.wantedBuildingUnits )
                {
                    ...
                    {
                        if ( HarrisonMapUtil.distance( wanted.location, HarrisonMapUtil.position( w.getFarm() ) ) < 2 )
                        {
                            return false;
                        }
                    }
    ...
    protected class BuildBuildingAction extends Action
    {
        ...
        @Override
        public void execute( HarrisonUnitController unit )
        {
            HarrisonBuilderUnitController builder = ( HarrisonBuilderUnitController ) unit;
            HarrisonWantedUnit wanted = nextBuildingToBeBuilt();
            if ( wanted != null && ai.harrisonBuildingManager.canAffordBuilding( wanted.unitType ) )
            {
                ...
                { // is possible to reach goal
                    if ( path.size() != 0 )
                    {
                        ...
                        {
                            builder.addAction( new UnitAction( builder.unit, UnitAction.MOVE, path.get( i )[0] % HarrisonMapUtil.WIDTH, path.get( i )[0] / HarrisonMapUtil.WIDTH, -1 ),
                                    HarrisonMapUtil.harrisonTrafficMap, path.get( i )[0], path.get( i )[1], path.get( i )[1] + builder.unit.getMoveSpeed() );
                        }
                        // first item in path is the planned location
                        int position = path.get( 0 )[0];
                        int time = path.get( 0 )[1];
                        
                        builder.addAction( new UnitAction( builder.unit, UnitAction.BUILD, position % HarrisonMapUtil.WIDTH, position / HarrisonMapUtil.WIDTH, wanted.unitType ),
                                HarrisonMapUtil.harrisonTrafficMap, position, time, time + builder.getBuildSpeed() );
    ...
    protected class RequestNewStockpileAction extends Action
    {
        ...
        public void execute( HarrisonUnitController unit )
        {
            ...
            else if ( y >= HarrisonMapUtil.HEIGHT )
            {
    ...
    protected class WorkerReturnResourcesAction extends Action
    {
        ...
        @Override
        public void execute( HarrisonUnitController worker )
        {
            HarrisonWorkerUnitController w = ( HarrisonWorkerUnitController ) worker;
            ...
            if ( path != null )
            {
                for ( int i = path.size() - 1; i >= 1; i-- )
                {
                    worker.addAction( new UnitAction( worker.unit, UnitAction.MOVE, path.get( i )[0] % HarrisonMapUtil.WIDTH, path.get( i )[0] / HarrisonMapUtil.WIDTH, -1 ),
                            HarrisonMapUtil.harrisonTrafficMap, path.get( i )[0], path.get( i )[1], path.get( i )[1] + worker.unit.getMoveSpeed() );
                }
                // first element is the stockpile
                int position = path.get( 0 )[0];
                time = path.get( 0 )[1];
                worker.addAction( new UnitAction( worker.unit, UnitAction.RETURN, position % HarrisonMapUtil.WIDTH, position / HarrisonMapUtil.WIDTH, -1 ), HarrisonMapUtil.harrisonTrafficMap,
                        position, time, time + UnitAction.DEFAULT_COOLDOWN );
            }
            else
            {
                for ( UnitAction action : worker.unit.getActions() )
                {
                    ...
                    {
                        worker.addAction( action, HarrisonMapUtil.harrisonTrafficMap, HarrisonMapUtil.position( action.getTargetX(), action.getTargetY() ), ai.currentTurn,
                                ai.currentTurn + w.getMoveSpeed() );
                        break;
                    }
                }
            }
    ...
    protected class WorkerHarvestAction extends Action
    {
        ...
        private HarrisonFarmUnitController nextFreeFarm()
        {
            ...
            {
                if ( farm.isFree() && HarrisonMapUtil.getSurroundingPositions( HarrisonMapUtil.position( farm ) ).size() > 0 )
                {
                    return farm;
                }
            }
